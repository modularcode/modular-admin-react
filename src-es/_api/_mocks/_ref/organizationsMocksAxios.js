...
export default {
  init(mock, instance) {
    ...
    //
    mock.onGet(/\/organizations\/\d+/).reply((config) => {
      // console.log(config)
    ...
    mock.onPut(/\/organizations\/\d+/).reply((config) => {
      const urlPaths = config.url.split('/')
      const organizationId = urlPaths[urlPaths.length - 1]
    ...
    mock.onPost(/\/organizations/).reply((config) => {
      const organizationData = JSON.parse(config.data)
      return [200, { id: 100, ...organizationData }]
    })
    mock.onDelete(/\/organizations\/\d+/).reply((config) => {
